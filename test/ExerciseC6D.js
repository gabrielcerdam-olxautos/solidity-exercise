...
contract("ExerciseC6D", async (accounts) => {
  ...
  before("setup contract", async () => {
    ...
    // commented because events.watch is not exist
    const ON_TIME = 10;
  ...
  it("can register oracles", async () => {
    // ARRANGE
    let registrationFee = await config.exerciseC6D.REGISTRATION_FEE.call();
    let fee = web3.utils.toWei(
      web3.utils.fromWei(registrationFee, "ether"),
      "ether"
    );
    // ACT
    console.log("REGISTRATION_FREE", fee);
    for (let a = 1; a < TEST_ORACLES_COUNT; a++) {
      console.log(
        "balance account" + accounts[a],
        await web3.eth.getBalance(accounts[a])
      );

      let estimateGas = await config.exerciseC6D.registerOracle.estimateGas({
        from: accounts[a],
        value: fee,
      });

      console.log("estimateGas to registreOracle: ", estimateGas);

      let transaction = await config.exerciseC6D.registerOracle({
        from: accounts[a],
        value: fee,
      });
      ...
      console.log(`Oracles: ${result[0]}, ${result[1]}, ${result[2]}`);
    }
  });
  ...
  //     let oracleIndexes = await config.exerciseC6D.getOracle(accounts[0]);
